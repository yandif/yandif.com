...
BASE 常用于描述 NoSQL 数据库的特性。与 CAP 定理相比，BASE 选择了可用性（Availability）而不是一致性（Consistency）。
...
HTTP 是一个应用层协议，依赖于诸如 TCP 和 UDP 等更底层的协议进行通信。
...
1. 确定资源（在 HTTP 中使用 URI）- 无论执行任何操作，使用相同的 URI。
